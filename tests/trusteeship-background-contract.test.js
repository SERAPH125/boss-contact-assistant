...
test('composes the live drill after the production engine and passes it to runtime', () => {
  const engine = background.indexOf('monitorEngine = MonitorEngine.create({');
  const creation = background.indexOf('TrusteeshipLiveDrill.create({');
  const controller = background.indexOf('TrusteeshipRuntime.createController({');
  assert.ok(engine > 0 && creation > engine && controller > creation);
  ...
  assert.match(background.slice(controller, controller + 800), /liveDrill:\s*trusteeshipLiveDrill/);
});
