...
## 2026-07-28 Chrome 商店发布边界

Chrome 应用商店发布链路只收敛运行平台、权限和发布文件，不改变 AI 托管行为。用户确认保留开启全局与单会话托管后的无人逐条确认自动外发；策略白名单、随机延时、静默时段、普通回复额度、明确拒绝单次结束、人工待确认和持续监控状态机均保持原样。

发布回归执行 `node --check src/background.js`、`node --check src/sidepanel.js` 和 `npm test`，结果为 460/460 通过。完成前校验又发现系统 `zip -X` 仍会保留每次复制产生的成员时间，导致跨时间构建 SHA 不同；失败用例跨越 ZIP 两秒粒度稳定复现后，打包器把 37 个白名单成员的权限固定为 `0644`、时间固定为 `1980-01-01 00:00:00 UTC`，并以稳定顺序压缩。修复后跨时间两次构建 SHA-256 都是 `1537017859dfcd42b1904935bf95f21316e4172c13db0e652caa2ded6bdb4190`，完整性检查通过。发布包不包含测试、文档、开发脚本或本地凭据。

本轮未修改 `trusteeship-policy.js`、`monitor-engine.js`、`trusteeship-runtime.js`、`reply-ai.js` 和 Boss 发送协议。自动外发能力在隐私政策与商店审核说明中被明确披露；这可能造成商店拒审，但发布脚本不会为了规避审核而静默关闭功能。
