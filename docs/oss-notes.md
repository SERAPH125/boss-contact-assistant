...
自动化回归覆盖：两个官方目录的金华代码解析、远程目录持久缓存、未知城市失败关闭、两平台精确职位描述节点、BOSS 顺序读取与稳定 ID、虚拟列表批次、致命页面门禁、三态筛选、AI 缺正文降级、外部动作重新绑定目标、拟人点击延迟竞态、遗留继续沟通弹窗、托管计划与逐岗位前置检查、规范会话登记及 Manifest 加载顺序。2026-07-30 的 ego-lite 只读验收在同一已登录 BOSS 搜索页分批检查 12 个岗位，12/12 均取得目标稳定 ID 和非空正文，页面未出现验证码、操作频繁或账号异常；全过程没有勾选岗位、点击联系、发送消息或修改托管状态。
