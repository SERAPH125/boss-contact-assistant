...
旧扫描只保存职位卡片字段，包含词、排除词和 AI 岗位判断都看不到职位描述；智联 `OPEN_JD` 甚至只是重新拼接卡片文本。第一版共享 `job-description` 适配层又错误地把 BOSS 与普通详情页等同，批量直连多个 `/job_detail/`：真实页面前 3 个返回正文，之后虽然 HTTP 状态仍为 200，实际内容却是约 41KB 的“请稍候 / BOSS正在加载中”软加载页。扩展因而把其余职位描述标成读取失败，并在旧二态 UI 中等同于淘汰，造成“扫描 40 个、只剩 3 个”的假象。这种软加载证据说明读取方式触发了平台保护或节流，但没有验证码、账号异常或操作频繁提示时，不能据此断言账号被封控。

修复继续保留共享归一化层，但 BOSS 改用其搜索页已有的 SPA 详情面板：在每批卡片仍可见时逐个点击，等待 `.job-detail-container .more-job-btn` 中的稳定岗位 ID 与目标完全一致，再从同一个可见详情 root 组合标题、招聘者、`.job-detail-body .job-label-list li` 与 `.job-detail-body .desc`。隐藏旧面板不会与当前面板拼接字段。读取完成当前批次后才滚动发现下一批，兼容虚拟列表；相似标题、空标题、旧面板未变化均不能通过校验。登录或明确阻断会中止整批，暂时读取失败则进入第三态“待核对”，默认不选但允许人工查看，且禁止 AI 把缺少正文的岗位推荐为安全匹配。智联仍使用受限并发的独立详情读取。每个正文最多保留 6000 字。

外部联系路径同步吸收了 [boss-agent-cli 的稳定 `encryptJobId` 模型](https://github.com/can4hou6joeng4/boss-agent-cli/blob/master/src/boss_agent_cli/api/models.py)、[同一岗位双详情通道](https://github.com/can4hou6joeng4/boss-agent-cli/blob/master/src/boss_agent_cli/commands/detail.py)，以及 [GeekGeekRun 逐卡点击并等待目标详情响应](https://github.com/geekgeekrun/geekgeekrun/blob/master/packages/geek-auto-start-chat-with-boss/index.mjs) 的架构经验：职位描述缓存只用于筛选，`GO_CHAT` 仍必须重新激活并核对目标稳定 ID，不能因为缓存命中就对当前旧面板点击“立即沟通”。项目没有复制开源源码，也没有使用其私有接口或绕过机制。

“联系已选并开启 AI 托管”也不再只依赖按钮文案：生成一次性交付计划时先只读检查 API 证明、回复依据、飞书证明、风险确认与全局运行状态，并在每个岗位开始处理和紧邻 `GO_CHAT` 前再次检查；联系成功后只有规范会话 ID、精确聊天 URL 和初始消息指纹齐全才报告托管成功。联系已发生但元数据不足时保留联系结果并明确报告托管未开启，终态拒绝会话不会被重新启用。

外部动作复核进一步发现五个独立错绑窗口：详情读取与拟人延时期间托管前置条件可能失效；全局查找“立即沟通”不能证明按钮属于目标详情；进入聊天页后，“公司 + 岗位”可能掩盖 HR 已经不同；拟人点击的 180–520ms 等待期间 SPA 仍可能切换详情；页面上原本可见的遗留“继续沟通”弹窗也可能被误认为本次联系产生。修复因此在 `GO_CHAT` 前紧邻外部动作再次执行托管只读前置检查，只允许点击已核验详情 root 内的动作按钮，并锁定该详情的岗位 ID、标题、公司和 HR；详情 HR 缺失时不再用卡片字段替代证明。`Humanize.humanClick` 的同步 `beforeDispatch` 守卫会在 mouseover、mousedown、mouseup 与 click 各阶段的实际事件派发前再次核对锁定身份、root、按钮归属、可见性和“无遗留继续沟通弹窗”。若 mousedown 已派发后证据才失效，结果按发送未知收束并停止整批，不会假定无副作用。首次点击前若已经存在任何可见“继续沟通”弹窗则整次联系失败关闭；无遗留弹窗时，之后也只接受本次点击后新出现且唯一的弹窗，并在确认事件实际派发前再次重证，多个弹窗一律不确认。详情招聘者会传给聊天页；初次发送前先解析 canonical peer，至少要求两个身份字段且全部已提供的 HR、公司、岗位字段都匹配，resolved peer 也必须与原目标交叉一致，不能覆盖原目标后再自证。图片、文字与 Enter 前均重新验证同一 peer 和同一 owned DOM scope；通用弹窗清理不再点击“继续沟通 / 确认”等动作按钮。

2026-07-30 发布前实测又发现上述 HR 安全门会 100% 停止联系：真实详情招聘者已经迁移到 `.job-boss-info h2.name`，旧生产选择器只覆盖 `.boss-name / .boss-info .name`；职位 ID、正文和按钮均已正确锁定，但 `hrName` 恒为空。这不是平台风控，而是详情适配器与现网 DOM 漂移。修复将卡片招聘者和详情招聘者拆成独立选择器，在已核验的唯一详情 root 内收集招聘者姓名、去除活跃状态、按规范值去重；“2月内活跃 / 2周内活跃 / 近半年活跃 / 4月前活跃”等完整状态后缀也会整体移除，不会把数字残留进 HR 身份。零候选或多个冲突姓名仍在任何点击前停止，且绝不回退到卡片字段猜测。测试夹具同时改为加载真实 `selectors.js`，新增现网 `.job-boss-info`、冲突姓名失败关闭、同名重复节点去重和活跃状态变体回归，避免测试硬编码旧 `.boss-name` 再次掩盖生产漂移。ego-lite 只读复核中，新选择器在当前唯一详情 root 内只命中一个 `H2.name`，并正确把“姓名 + 在线”规范为纯姓名；没有点击“立即沟通”或产生站外写入。

单次发送继续遵守本地知识库“外部副作用结果未知不得盲重放”的既有结论：Enter 后若输入框清空和新 outgoing 气泡均未出现，结果收束为 `SEND_RESULT_UNKNOWN`，不会再点击发送按钮；后台同时识别 `unknown` 与 `sendResultUnknown` 两种边界返回并停止本批次。这里新增的结论是，稳定岗位 ID 只能保护搜索页动作，聊天页仍需独立的 canonical 会话身份和作用域证明；两层证据不能互相替代。

## v0.3.6 联系成功但会话登记失败 / 联系并托管无反馈

2026-07-30 实号日志与 ego-lite 复核确认两组独立问题：

1. **登记失败「稳定联系人身份与原目标不一致」**  
   对照 [browser-harness BOSS job card](https://github.com/browser-use/browser-harness/blob/main/agent-workspace/domain-skills/BOSS-zhipin/chat.md) 与现网 DOM：列表卡 `span.boss-name` / `a.boss-info` 是**公司名**，不是招聘者；招聘者只在详情 `.job-boss-info .name`。旧 `parseCard` 把公司写入 `hrName`、公司字段留空；好友列表又常缺 `jobName`，且 `brandName` 带 `…` 截断。`CAPTURE_CONTACTED_CONVERSATION` 在 DOM 已严格匹配后仍要求好友结构化字段 ≥2 条证据，因而只剩 HR 时失败关闭，招呼已由 BOSS 发出也不会重发。  
   修复：列表卡只把 `.boss-name` 当作公司；详情公司优先解析 `.boss-info-attr`（`公司 · 角色`）；身份比对去掉省略号；DOM 已匹配且 peer 唯一收敛时，好友侧 1 条稳定字段即可佐证。

2. **「联系已选并开启 AI 托管」看似无反应**  
   该入口在生成确认单前要求全局 AI 托管已开启且未暂停（另需 API/飞书/回复依据证明）。失败原先只写入「执行」页日志，岗位筛选页无可见反馈。现于筛选页 `deliveryStatus` 展示错误与下一步，并保留执行日志。

自动化覆盖：省略号公司匹配、ellipsis brandName 登记、筛选页准备失败可见提示、`.greet-boss-dialog` 容器无 offsetParent 时仍保护成功回执。

自动化回归覆盖：两个官方目录的金华代码解析、远程目录持久缓存、未知城市失败关闭、两平台精确职位描述节点、BOSS 顺序读取与稳定 ID、虚拟列表批次、致命页面门禁、三态筛选、AI 缺正文降级、外部动作重新绑定目标、拟人点击延迟竞态、遗留继续沟通弹窗、托管计划与逐岗位前置检查、规范会话登记及 Manifest 加载顺序。2026-07-30 的 ego-lite 只读验收在同一已登录 BOSS 搜索页分批检查 12 个岗位，12/12 均取得目标稳定 ID 和非空正文，页面未出现验证码、操作频繁或账号异常。

2026-07-30 续：ego-lite 实号全流程验收（阜阳 · 跨境电商运营）确认：

1. 列表卡 `.boss-name` 实为公司名（旧逻辑会误写入 `hrName`）；详情 `.job-boss-info h2.name` 才是招聘者。
2. 好友列表普遍无 `jobName`，`brandName` 常带 `…`，`title` 是「人事 / 招聘者」等职务。
3. 对历史失败会话「孙先生 / 颍泉区左岸服装商…」：DOM 严格匹配 + 好友侧省略号公司佐证均通过。
4. 对未联系岗位「吉林省萌敬商贸 / 吕程」真实点击「立即沟通」→ 出现 `.greet-boss-dialog`（容器 `offsetParent` 可为 null，内部 `.sure-btn` 可见）→「继续沟通」进入聊天页；平台已发出首条招呼；好友收敛到 `encryptUid`，只读 `historyMsg` 成功，`registrationWouldSucceed=true`。未在聊天页二次填写输入框。
...
## 2026-07-30 BOSS 首次联系成功弹窗与 AI 托管登记

本轮继续对照 [GeekGeekRun 的联系事务](https://github.com/geekgeekrun/geekgeekrun/blob/master/packages/geek-auto-start-chat-with-boss/index.mjs#L1521-L1640)（ISC）：首次联系需要独立成功证据，前置确认和结果弹窗属于不同阶段；并参考 [boss_batch_push 的 `friendStatus` 判断](https://github.com/yangfeng20/boss_batch_push/blob/master/src/oop-self-req-main.js#L1151-L1157)、[成功响应后再落缓存](https://github.com/yangfeng20/boss_batch_push/blob/master/src/oop-self-req-main.js#L1389-L1400)及其[按钮状态可能滞后的说明](https://github.com/yangfeng20/boss_batch_push/blob/master/README.md#L84-L90)（Apache-2.0）。项目只采用“外部动作与后续会话捕获分阶段、成功依赖正证据”的架构结论，没有复制源码或使用平台私有发信协议。

ego-lite 对当前 BOSS 生产 bundle 的只读检查确认，首次建联成功后出现 `.greet-boss-dialog`，标题为“已向BOSS发送消息”，关闭图标是 `.icon-close`，“继续沟通”按钮是 `.sure-btn`；点击后才导航到 `/web/geek/chat?...`。旧实现先运行通用 `[class*="dialog"] .icon-close` 清理，恰好在识别“继续沟通”前关闭成功弹窗，随后以 `success:true,navigated:false` 返回。后台又把任意非失败返回视为成功，提前写入日限和去重；最终虽然 BOSS 可能已发送首条招呼，页面却未进入聊天页，无法取得 canonical 会话引用，因此“联系已选”和“联系已选并开启 AI 托管”都会失败，后者还必然无法开启托管。这不是验证码或平台风控。

修复把带业务动作的弹窗排除在所有通用关闭规则之外，并按 `.greet-boss-dialog` 容器身份无条件保护成功回执：即使“继续沟通”尚未挂载、暂不可见或出现 A/B 文案变化，也不会点击其关闭图标或继续点击背后的“立即沟通”。实号还发现该容器自身 `offsetParent` 常为 null，但内部 `.sure-btn` / `.icon-close` 仍可见；因此保护条件改为“节点已连接 DOM，或内部动作/关闭节点可见”，并在通用 × 清理中显式跳过 `.greet-boss-dialog` 内的关闭图标。首次联系前先冻结遗留弹窗，点击后先等待本次唯一“继续沟通”，不再运行通用清理。父按钮和内部文字节点会先归一、去重为同一个可点击动作，防止站点增加嵌套标签后产生假歧义。确认成功后立即导航；既无成功回执也未进入聊天页时收束为不可自动重试的未知结果。

后台成功门同步收紧为必须存在 `navigated / inPageChat / skipChat / applied` 之一，或消息通道关闭后另行取得聊天 URL 正证据。`Receiving end does not exist`、`Could not establish connection` 和 `Extension context invalidated` 归类为“请求没有可靠接收端”，不再冒充导航成功；一般消息端口关闭也必须先核验 URL，未进入聊天页则按结果未知停止。孤立的 `success:true` 或消息错误不再占用日限、写入去重或宣称托管成功。

两个岗位筛选入口共用上述联系链路。进一步的现网只读证据表明，“已向BOSS发送消息”出现时平台已经发出了首条招呼；旧后台进入聊天页后仍调用 `SEND_ACTIVE`，实际上是在尝试第二次发送。新链路把后续步骤改为独立的 `CAPTURE_CONTACTED_CONVERSATION` 只读事务：严格核对可见 HR、公司、岗位，解析 canonical `conversationRef`，读取初始消息指纹，再由“联系已选”登记为 `enabled:false`、由“联系已选并开启 AI 托管”登记为 `enabled:true`。捕获失败保留联系成功事实，不重放招呼，也不因只读登记失败停止整个联系批次。

ego-lite 对当前 BOSS 好友列表接口的只读检查还确认，真实记录通常只有 `name`、`brandName`、`title` 等字段，其中 `title` 是“招聘者 / 招聘专员 / 人事培训”等招聘者职务，不是岗位名；记录可能完全没有 `jobName` 或 `positionName`。旧实现要求稳定接口同时返回 HR、公司和岗位，必然把正常会话误判为 `TARGET_UNCERTAIN`。修复后，可见聊天页仍严格校验 HR、公司、岗位；稳定好友记录只比较其实际存在的可靠 HR 与公司，缺失岗位由已核验页面补全，任何明确字段冲突仍失败关闭。可执行 background VM 回归覆盖两个入口的只读登记、通道关闭后的安全重试、捕获失败不重放；聊天页 fake DOM 回归覆盖稀疏好友元数据成功登记、HR 冲突零写入和招聘者职务不冒充岗位。

本轮复用的本地知识库笔记为 `技术复用/浏览器扩展-AI会话托管可靠性复盘.md`。既有结论是“外部副作用结果未知不得盲重放、成功必须有正证据、canonical 会话身份才能驱动托管”；本轮新增结论是“平台成功回执与托管会话捕获是相邻但独立的两个事务，成功回执之后只能做可安全重试的只读捕获；上游稳定身份接口缺字段时应比较已有可靠字段，而不是把缺失误判为冲突”。
