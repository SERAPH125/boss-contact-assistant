...
> 状态：**M1/M2 已落地**；**M3 智联适配器 MVP 已落地**（列表扫描 + 沟通/投递）；P0 批次信任闸门自动验收已通过；Boss/智联真机验收待授权；猎聘待 M4  
...
先支持 **Boss（已有）→ 智联 → 猎聘**；不做全自动海投、不做多平台并行队列。
...
└──────────────┘                  └────────┬────────┘
                    ...
                    ┌──────────────────────┼──────────────────────┐
                    ▼                      ▼                      ▼
            platform/boss/*        platform/zhilian/*      platform/liepin/*
            selectors+content      selectors+content       selectors+content
            (已有，整理迁入)         (新建，借调研)           (新建，借调研)
```
...
```text
activePlatform: 'boss' | 'zhilian' | 'liepin'
...
byPlatform: {
  ...
  liepin:  { ...公共, salaryYearMin, salaryYearMax, preferUrgent,
             contactDay, contactCount, processed{} }
}
...
| | Boss（现网） | 智联 | 猎聘 |
|--|-------------|------|------|
| 主站 | zhipin.com | zhaopin.com | liepin.com |
| 主操作 | 立即沟通 | 立即申请 / 聊一聊（真机确认） | 立即沟通 / 投递（真机确认） |
| 上限注意 | 沟通次数 / 验证码 | 日投上限约百级；需默认简历（get_jobs） | 打招呼与主动消息限额不同（get_jobs） |
| MVP 联系语义 | 沟通 + 招呼语 | 优先「聊一聊/沟通」路径；申请类若需简历附件则 Phase 2 | 优先沟通；避免触发主动私信上限 |
| 选择器 | 已有 | Phase Z 真机录制 | Phase L 真机录制 |
...
- [x] 你确认本设计方案无大改
- [ ] Clone/浏览对照：JobsIn（adapter 形状）、get_jobs 智联/猎聘说明、BossAssistant
...
- [x] 未登录横幅模板化
- [x] 筛选页按平台显示专属字段（智联/猎聘字段可先占位，adapter 未上线时禁用扫描并提示）
...
### Phase M4 · 猎聘适配器 MVP（2–4 天）

- [ ] manifest：`*://*.liepin.com/*`
- [ ] `platform/liepin` 同上
- [ ] 借鉴 get_jobs：打招呼策略提示；避免误触主动消息上限
- [ ] 真机验收同智联清单
...
优先顺序建议：**M1 → M2 → M3 → M4**（先抽层再铺平台，避免改三次 UI）。

---

## 9. 验收标准（多平台 DoD）

1. 名称显示为「求职联系助手」；配置页三卡片可选 Boss / 智联 / 猎聘  
2. 同时只能激活一个平台；切换后勾选清空，审核列表不混平台  
3. API Key 全局一份；招呼语与日限 **每平台独立** 且刷新后仍在  
4. 未登录横幅同款样式，host 随平台变化  
5. Boss 原有能力不回退；智联、猎聘各自可完成「扫描 → 勾选 → 联系已选」  
...
- 猎聘仍为 `ready: false`，既没有联系真机验收，也没有托管适配。

因此 Phase 1 不把 Boss 选择器或发送协议抽象成“看似通用”的智联/猎聘实现，也不在其他平台显示可用托管开关。若未来扩展平台，必须分别完成稳定会话 ID、owned DOM scope、发送肯定证据、登录/风控分类和真实账号分阶段验收；不能仅复用 UI 或状态机就宣称平台支持。
